	...
	$('#run-search').on('click', function(event) {
		...
		$.ajax({
			url:
				...
				city +
				'&client_id=%20MTU3Mjk3MzV8MTU1MjQxMjMwMS40Nw&client_secret=1a02aa5dd9e60be247c7c822cb6b46a165cbc902e448434afcf44d242ebde6f3',
			method: 'GET'
			...
			for (var i = 0; i < eventList.length; i++) {
				...
				var eventURL = $("<a target= '_blank'>")
					.attr('href', event.venue.url)
					.text('Link');
			...
			function storeEvent(str) {
				...
				db.ref('users/' + userId + '/event/').push(newEvent);
			}
